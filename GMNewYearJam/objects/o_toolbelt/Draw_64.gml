if active 
{
	...
	for (var i = 0; i < len; i++)
	{
		for (var k = 0; k < len; k++)
		{
			...
			color = c_white;
			//Checks if hovered 
			if toolArray[@ i,0] == toolstate.hover
			{
				alpha = 1;
				color = c_orange;
			}
			//Draws Tool UI sprites
			draw_sprite_ext(toolsprites[i],0,startX+i*cellsize,startY,2,2,0,c_white,alphaGL);
			//Draws Grid
			draw_sprite_ext(s_grid,0,startX+i*cellsize,startY,4,4,0,c_white,alphaGL);
			draw_sprite_ext(s_brown_container_shell,0,startX+i*cellsize,startY,1,1,0,color,alphaGL);
